        ...
        try:
            agg_result = (
                ...
                .agg([
                    ...
                    pl.col("mcc_code").n_unique().alias(f"mcc_uniq_{window_name}")
                    if "mcc_code" in feat.columns else pl.lit(0).alias(f"mcc_uniq_{window_name}"),

                    pl.lit(0).alias(f"country_uniq_{window_name}"),
...
def add_graph_features(
    ...
    result = df_c.join(merch_stats, on="pos_cd", how="left")
    return result.lazy()
...
def build_client_pretrain_profile(pretrain: pl.LazyFrame) -> pl.DataFrame:
    ...
    return (
        ...
        .agg([
            ...
            # Уникальные категории
            pl.col("mcc_code").n_unique().alias("pretrain_uniq_mcc")
            if "mcc_code" in pretrain.columns else pl.lit(0).alias("pretrain_uniq_mcc"),

            pl.lit(0).alias("pretrain_uniq_countries"),
